...
class Command(BaseCommand):
    ...
    def list_model_info(self, options):
        ...
        FIELD_CLASS = (
            True if options.get("field_class") is not None else getattr(settings, "MODEL_INFO_FIELD_CLASS", False)
        )
        DB_TYPE = True if options.get("db_type") is not None else getattr(settings, "MODEL_INFO_DB_TYPE", False)
        SIGNATURE = (
            True if options.get("signature") is not None else getattr(settings, "MODEL_INFO_SIGNATURE", False)
        )
        ALL_METHODS = (
            True if options.get("all_methods") is not None else getattr(settings, "MODEL_INFO_ALL_METHODS", False)
        )
        MODEL = (
            options.get("model")
